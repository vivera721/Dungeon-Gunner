...
public class ActiveWeapon : MonoBehaviour
{
    ...
    private void SetWeapon(Weapon weapon)
    {
        ...
        // weapon 이 polygon collider 와 sprite 를 가지고 있으면 weapon physics shape 로 설정
        if (weaponPolygonCollider2D != null && weaponSpriteRenderer.sprite != null)
        {
            // Get sprite physics shape - this returns the sprite physics shape points as a list of Vector2s
            // 스프라이트 물리 형상 가져오기 - 2차원 Vector의 목록으로 스프라이트 물리 형상 지점을 반환합니다.
            ...
            // 스프라이트에 대한 물리 형상을 선택하도록 무기에 polygon collider 설정 - collider 점을 스프라이트 물리 형상 지점으로 설정
